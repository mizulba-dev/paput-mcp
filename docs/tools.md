...
profile_summary (multi-line overall summary), and strength_labels (array of strength
items with optional description, category names, and project IDs) fields generated by
the MCP client AI model. It should be used only after the AI model has generated a
public profile and the user intends to save it.
...
so the MCP client AI model can generate the headline, profile_summary, and
strength_labels shown on the AI Summary tab. It intentionally excludes private
dashboard analysis and goals. Present the draft first and save with

...
category_names must be EXACT names taken from structuredContent.categories — do not invent, translate, or paraphrase category names. If no listed category matches the item, leave category_names empty; categories are a secondary lens, so an empty list is better than an approximate name.`;
}
